...
describe(Class, function () {
    ...
    describe('subscribe', function () {
        ...
        it('should not automatically publish when guarded', function () {
            expect(A1.on).not.toHaveBeenCalled();
            expect(A2.on).not.toHaveBeenCalled();
            expect(B1.on).not.toHaveBeenCalled();
        });
        ...
        it('should automatically publish when unguarded', function () {
            c.publish(7, 8, 9);
            c.subscribe(C1, 'on');
            expect(C1.on).toHaveBeenCalledWith(7, 8, 9);
        });

        it('should not republish to subscribers', function () {
            c.subscribe(C2, 'on');
            c.publish(7, 8, 9);
            ...
            c.subscribe(C1, 'on');
            expect(C1.on).toHaveBeenCalledWith(7, 8, 9);
            expect(C2.on).not.toHaveBeenCalled();
        });
    });
});
